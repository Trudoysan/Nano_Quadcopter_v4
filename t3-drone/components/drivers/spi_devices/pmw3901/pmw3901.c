...
bool pmw3901Init(uint32_t csPin)
{
    ...
    ESP_LOGI("pmw3901Init","Motion chip id: 0x%x:0x%x\n", chipId, invChipId);
...
void readMotionCount(int16_t *deltaX, int16_t *deltaY,uint32_t csPin)
{
  registerRead(csPin, 0x02);
  *deltaX = ((int16_t)registerRead(csPin, 0x04) << 8) | registerRead(csPin, 0x03);
  *deltaY = ((int16_t)registerRead(csPin, 0x06) << 8) | registerRead(csPin, 0x05);
}
